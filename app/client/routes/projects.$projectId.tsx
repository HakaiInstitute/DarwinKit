...
function ProjectIndexComponent() {
  const projectId = Route.useParams({
    select: (d) => z.number().parse(d.projectId),
  });
  const projectQuery = trpcReact.projectWithFiles.useQuery({ id: projectId });

  return (
    <div
      id="chat"
      className="flex flex-col items-center justify-center min-h-dvh h-full mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8"
