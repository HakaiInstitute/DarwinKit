...
export async function seedVocabularies() {
  ...
  await db.insert(standardFields).values([
    ...
    },
    {
      ...
      description:
        "The reproductive condition of the biological individual(s) at the time of the Occurrence",
      primitiveType: "string",
      semanticType: "controlled_vocabulary",
      controlledVocabularyId: lifeStageVocabulary.id,
  ...
  logger.log("✅ Darwin Core vocabularies seeded successfully!");
}
...
if (require.main === module) {
  seedVocabularies()
    .then(() => process.exit(0))
    .catch((error) => {
      logger.error("❌ Error seeding vocabularies:", error);
      process.exit(1);
    });
}
