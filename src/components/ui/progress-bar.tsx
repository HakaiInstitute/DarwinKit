            ...
            <div key={step.name} className={clsx("relative flex gap-2 p-4")}>
              <div className="">
                {step.status === "complete" && (
                  <Icon icon="check" className="h-5 w-5 text-green-500 dark:text-green-400" />
                )}
              </div>
              <div
                className={clsx("min-w-0", {
                  "text-green-700 dark:text-green-400": step.status === "complete",
                })}
              >
                <p className="text-xs">
                  {index + 1}. {step.name}
                ...
                {step.status == "complete" ? (
                  <span className="text-xs text-green-600 dark:text-green-400">{step.result}</span>
                ) : (
