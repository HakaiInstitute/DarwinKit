...
function IndexComponent() {
  ...
  const createProjectMutation = useMutation(
    orpc.project.create.mutationOptions({
      onSuccess() {
        ...
        //   queryKey: orpc.project.list.queryKey({ queryKey: [""] }),
        // });
      },
      onError(error) {
        console.error(error);
  ...
  return (
    <div className="mx-auto min-h-dvh flex flex-col items-stretch justify-center max-w-4xl xl:max-w-7xl px-4 sm:px-6 xl:px-8">
      ...
      <p className="text-lg mb-6">Welcome to DarwinKit, a biodiversity mapping tool.</p>
      ...
      <div className="mb-6">
        ...
        {projectsQuery.error && (
          <p className="text-red-600">Error loading projects: {projectsQuery.error.message}</p>
        )}
        {projectsQuery.data && (
          <div className="space-y-3">
            {projectsQuery.data.length === 0 ? (
              <p className="text-gray-600 italic">No projects yet. Create your first project!</p>
            ) : (
