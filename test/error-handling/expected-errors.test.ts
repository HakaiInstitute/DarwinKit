...
Deno.test("Expected errors - all catchable with Effect.catchAll", async (t) => {
  await withTestDirectory(async (tempDir) => {
    ...
    await t.step("Invalid CSV data", async () => {
      ...
      await writeCsvFile(tempDir, "data", [{ col1: "val1", col2: "val2" }]);
    ...
    await t.step("Invalid workspace configuration", async () => {
      // Create invalid darwinkit.json in a subdirectory
      const configDir = join(tempDir, "invalid-config");
      await Deno.mkdir(configDir, { recursive: true });
      await writeJsonFile(configDir, "darwinkit.json", "{ invalid json }");

      let errorCaught = false;

      await Effect.runPromise(
        Workspace.discover(configDir).pipe(
