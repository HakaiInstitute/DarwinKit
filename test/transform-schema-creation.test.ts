...
Deno.test("createTableFromSchema - creates tables, enums, and constraints", async () => {
    ...
    try {
        ...
        const effect = await createTableFromSchema(connection, config);
        await Effect.runPromise(effect);

        // 3. Verify the results
        // Check if ENUM type for basisOfRecord was created for the Occurrence table
        const enumResult = await (await connection).runAndReadAll(
        ...
        const eventIdCol = eventColumns.find((c: any) => c.name === "eventID");
        ...
        const yearCol = eventColumns.find((c:  any) => c.name === "year");
        ...
        const occurrenceIdCol = occurrenceColumns.find((c: any) => c.name === "occurrenceID");
        ...
        const basisOfRecordCol = occurrenceColumns.find((c: any) => c.name === "basisOfRecord");
...
Deno.test("createTableFromSchema - handles complex schema with multiple tables and FKs", async () => {
    ...
    const config: WorkspaceConfig = {
        ...
        transform: {
            ...
            datasets: [
                ...
                { name: "MeasurementOrFact", profile: "dwc-extendedMeasurementOrFact", source: { test: "" }, fields: {} },
            ],
            output: {},
        },
    };
    ...
    try {
        ...
        // Verify Event table
        const eventInfo = await (await connection).runAndReadAll("PRAGMA table_info(event);");
        assert(eventInfo.getRowObjects().some((c) => c.name === "eventID" && c.pk), "event.eventID should be PK");

        // Verify Occurrence table
        const occInfo = await (await connection).runAndReadAll("PRAGMA table_info(occurrence);");
        assert(occInfo.getRowObjects().some((c) => c.name === "occurrenceID" && c.pk), "occurrence.occurrenceID should be PK");
        const basisOfRecordCol = occInfo.getRowObjects().find((c) => c.name === "basisOfRecord");
        assertExists(basisOfRecordCol, "basisOfRecord column should exist");
        assertEquals(basisOfRecordCol?.notnull, true, "basisOfRecord should be NOT NULL");

        // Verify MeasurementOrFact table
        const mofInfo = await (await connection).runAndReadAll("PRAGMA table_info(extendedmeasurementorfact);");
        assert(mofInfo.getRowObjects().some((c) => c.name === "measurementID" && c.pk), "mof.measurementID should be PK");

        // Verify Foreign Keys
        // 1. From Occurrence to Event
        const occFks = await (await connection).runAndReadAll("PRAGMA foreign_keys(occurrence);");
        ...
        const mofFks = await (await connection).runAndReadAll("PRAGMA foreign_keys(extendedmeasurementorfact);");
