...
(`feat:`, `fix:`, `feat!:`/`BREAKING CHANGE:` for majors). Two checks enforce
this on every PR: `commitlint` (the branch's individual commits) and `pr-title`
(the PR title). **PRs are squash-merged, so the PR title becomes the commit on
`main`** — it is what git-cliff reads to compute the changelog and the next
version. A non-conventional title (e.g. `Fix the thing` instead of `fix: the
thing`) means git-cliff finds "nothing to bump" and no release is cut. Both
checks should be marked **required** in `main`'s branch protection.
...
and mark the `commitlint` and `pr-title` checks required in branch protection. (The release
