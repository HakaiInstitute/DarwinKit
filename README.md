...
PR; merging it tags the release and builds the `dwkt` binaries. No release is
...
`release.yml` then builds and attaches the `dwkt-*` binaries automatically, so
`dwk_install_engine()` works. The tag/release step triggers the build only because
it runs under your credentials — the same step from CI needs `RELEASE_PAT`.
